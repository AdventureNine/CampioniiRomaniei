...
def test_quizz(quizz: Quizz, singleResponseQuestion: Question, multipleResponseQuestion: Question, fill_in_statement: FillInStatement, answers: list[str]):
    ...
    assert quizz.get_minigames()[0].get_image_path() == "backend/domain/assets/images/ui/coin.png"
    ...
    assert quizz.get_minigames()[2].get_theme() == "theme"
    ...
    assert quizz.get_minigames()[4].get_win_configuration() == {"ceva":(1,2), "ceva2":(2,1), "ceva3":(3,4), "ceva4":(4,3)}
...
def test_domain():
    ...
    bingo = Bingo(1, {"bingo_cell1_text": True, "bingo_cell2_text": False, "bingo_cell3_text": True, "bingo_cell4_text": False}, "theme")
    pairs = Pairs(1, {"question1": "answer1", "question2": "answer2", "question3": "answer3"})
    mapGuesser = MapGuesser(1, {"ceva":(1,2), "ceva2":(2,1), "ceva3":(3,4), "ceva4":(4,3)})
    quizz = Quizz(1, [singleResponseQuestion, multipleResponseQuestion], [fill_in_statement], [puzzle, rebus, bingo, pairs, mapGuesser], Difficulty[1])
