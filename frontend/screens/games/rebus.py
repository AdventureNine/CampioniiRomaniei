...
class RebusScreen(Screen):
    ...
    def build_grid(self, words_data, max_cols):
        ...
        fixed_width = self.number_w + (max_cols * self.cell_w)
        screen_width = Window.width
        padding_right_safety = dp(160)
        available_space_for_clue = screen_width - fixed_width - padding_right_safety
        actual_clue_w = max(self.clue_w, available_space_for_clue)
        grid.width = self.number_w + actual_clue_w + (max_cols * self.cell_w)
        ...
        for row_idx, word_info in enumerate(words_data):
            ...
            for col in range(max_cols):
                if col < word_info['start_col'] or col >= word_info['start_col'] + len(word_info['word']):
                    ...
                    if first_cell is None: first_cell = cell
                    if prev_cell:
